...
public unsafe struct AtelWorkerController {
    ...
    [FieldOffset(0x1E8)] public short npc_last_interaction; //TODO: Figure out a better name for this. This one comes from CSR
    [FieldOffset(0x1F8)] public  AtelScriptChunk* script_chunk;
    [FieldOffset(0x1FC)] private byte*            __0x1FC;

    public AtelBasicWorker* worker(int idx) {
        int missed = 0;

        if (idx < count_0xb58)
            return (AtelBasicWorker*)((nint)_workers + idx * 0xb58);

        idx -= count_0xb58;
        missed += count_0xb58 * 0xb58;

        if (idx < count_0x558)
            return (AtelBasicWorker*)((nint)_workers + idx * 0x558 + missed);

        idx -= count_0x558;
        missed += count_0x558 * 0x558;

        if (idx < count_0x5b8)
            return (AtelBasicWorker*)((nint)_workers + idx * 0x5b8 + missed);

        idx -= count_0x5b8;
        missed += count_0x5b8 * 0x5b8;

        if (idx < count_0x2e8)
            return (AtelBasicWorker*)((nint)_workers + idx * 0x2e8 + missed);
    ...
    public AtelBasicWorker* worker_b58(int idx) {
        if (idx < count_0xb58)
            return (AtelBasicWorker*)((nint)_workers + idx * 0xb58);
        throw new System.IndexOutOfRangeException($"Index {idx} is out of range for {count_0xb58} B58 workers.");
    }
}
